class GalleryController < ApplicationController
  ...
  def checkout
	amount_to_charge = session[:amount].to_i
	 ...
	 def search
		@perfumes_gallery = Store.find_by_sql ["Select * from stores WHERE name like ? or category like ? or color like ? or description like ?",params[:search],params[:search],params[:search],params[:search]]	  
	  end
	end
